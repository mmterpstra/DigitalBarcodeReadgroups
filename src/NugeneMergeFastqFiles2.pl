...
sub main {
	...
	my $use="$0 [-u] [-q INT] randombarcodes.fq.gz in_reads_1.fq.gz out_reads_1.fq.gz [in_reads_2.fq.gz out_reads_1.fq.gz].
...
now filters for illumina 1.8 base quality < -q INT barcode reads and N bases in randombarcodes.fq.gz";
	...
	if(not(defined($ARGV[2]))){
		die "Outfile1 does not exist! ".$ARGV[1]."\n".$use;;
	}
	
	#open handles fastq files
	
	open(my $fastq1Handle,"-|",'gzip -dc '.$ARGV[1]) 
	or die "[FATAL] Cannot read open fq1 file ".$ARGV[1];
	
	
	open(my $fastq1OutHandle,"|-",'gzip -c > '.$ARGV[2]) 
	or die "[FATAL] Cannot write fq1 file ".$ARGV[2];
	
	my $fastq2Handle;
	my $fastq2OutHandle;
	
	if(defined($ARGV[3]) && -e $ARGV[3] && defined($ARGV[4])){
		die "[FATAL] specify outfile2".$use if(not(defined($ARGV[4])));
		open($fastq2Handle,"-|",'gzip -dc '.$ARGV[3]) 
		or die "Cannot read open fq2 file ".$ARGV[3];
		
		open($fastq2OutHandle,"|-",'gzip -c >'.$ARGV[4]) 
		or die "Cannot write fq2 file ".$ARGV[4];
